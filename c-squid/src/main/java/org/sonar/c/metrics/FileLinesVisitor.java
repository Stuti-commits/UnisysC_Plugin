...
public class FileLinesVisitor extends CVisitor {
  ...
  private void visitComment(Trivia trivia) {
    String[] commentLines = CCommentAnalyser.getContents(trivia.getToken().getOriginalValue()).split("(\r)?\n|\r", -1);
    int line = trivia.getToken().getLine();
    for (String commentLine : commentLines) {
      if (commentLine.contains("NOSONAR")) {
        linesOfComments.remove(line);
        noSonarLines.add(line);
      } else if (!CCommentAnalyser.isBlank(commentLine) && !noSonarLines.contains(line)) {
