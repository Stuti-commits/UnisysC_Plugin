...
public class CProfileTest {

  @Test
  public void should_create_sonar_way_profile() {
    Context context = new Context();

    CProfile definition = new CProfile();
    definition.define(context);

    BuiltInQualityProfile profile = context.profile(C.KEY, CheckList.SONAR_WAY_PROFILE);

    assertThat(profile).isNotNull();

    List<BuiltInActiveRule> activeRules = profile.rules();
    assertThat(activeRules.size()).isGreaterThan(30);
